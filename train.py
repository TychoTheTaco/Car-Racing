...
def train(episodes: int = 1000, log_interval: int = 10, model_dir: str = 'models', save_interval: int = 100, buffer_size: int = 2000, batch_size: int = 128, gamma: float = 0.99,
    ...
    for episode in range(episodes):
        ...
        while not done:
            ...
            # Choose action
            p = model(np.expand_dims(observation, axis=0))[0][0]
            alpha, beta = p[:, 0], p[:, 1]
                ...
                for _ in range(ppo_epochs):
                    ...
                    for batch in gen_batches(indices, batch_size):
                        with tf.GradientTape() as tape:
                            ...
                            ab = model(tf.gather(states, batch))[0]
                            ...
                            one = model(tf.gather(states, batch))[1]
        ...
        if not episode % log_interval:
            print(f'Episode {episode} | Reward: {episode_reward:.02f} | 50-Episode Moving Average: {np.average(episode_rewards[-50:]):.02f}')
